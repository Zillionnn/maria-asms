...
const rdtlArea = {
  ...
  async insertOne(ctx) {
    ...
    let residentialAreaCount = await rdtlAreaModel.countByName(name);

    if (residentialAreaCount[0].count > 0) {
      util.handleError(ctx, "小区名称已存在");
    } else {
      await rdtlAreaModel
        .insertOne(body)
        .then(r => {
  ...
  async upload(ctx) {
    ...
    const stream = fs.createWriteStream(
      path.join(`/var/www/html/images`, name)
    );
    reader.pipe(stream);
